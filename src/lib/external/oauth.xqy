...
(: with modifications from https://github.com/adamfowleruk/feedsxquery/blob/master/src/app/models/oauth.xqy :)
(: which is itself from https://github.com/ndw/XQuery-OAuth :)
  ...
  $secret as xs:string?)
{
  ...
  let $noncei     := xdmp:hash64(concat(current-dateTime(),string(xdmp:random()))) (: Should this be an int increasing each time??? :)
  ...
  (: loop through OAuth parameters, both http headers and query string params, to create signature :)
  let $encparams
 ...
 (: realm=&quot;", $service/@realm, "&quot;,  :)
  let $authheader := concat("OAuth realm=&quot;", $service/@realm, "&quot;,",
   ...
   let $data     := ()
(:
                        <xh:Content-Type>application/x-www-form-urlencoded</xh:Content-Type>:)
